def selection_sort(arr):
    ...
    while place < length:
        print("array is ", arr)
        this_slice = arr[place:]
        print("sub array is", this_slice)

        smallest = min(this_slice)
        print("smallest number is", smallest)
        smallest_index = this_slice.index(smallest)+place
        print(f"{smallest} is located at {smallest_index}")
        holder = arr[place]
        print(f"The number at the lowest index ({place}) is {holder}")

        if place != smallest_index:
            print("SWAPPING NUMBERS")
            arr[place]= smallest
            print(f"{smallest} is now in {place}")
            arr[smallest_index] = holder
            print(f"{holder} is now in {smallest_index}")
        else:
