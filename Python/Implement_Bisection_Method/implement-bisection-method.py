def square_root_bisection(target,tolerance = 0.01,max_iter = 5):
    if target < 0:
    ...
    while tries < max_iter:
        this_guess = (high + low) / 2
        this_square = this_guess * this_guess


        if (high-low) < tolerance:
            print(f"The square root of {target} is approximately {this_guess}")
            return this_guess

        elif this_square < target:
            low = this_guess
        else:
            high = this_guess
