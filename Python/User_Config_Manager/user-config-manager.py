...
def update_setting(settings, change_setting):
    key = change_setting[0].lower()

    value = change_setting[1].lower()
...
def view_settings(settings):
    ...
    returnString = "Current User Settings:\n"
    for key,value in settings.items():
        returnString += f"{key.capitalize()}: {value}\n"
    print (returnString)
    return returnString
