class HashTable:
    ...
    def lookup(self,key):
        print("start looukp")
        key_hash=self.hash(key)
        print(f"Looking for {key} as {key_hash}")
        print(key_hash in self.collection)
        if key_hash in self.collection:
            print(f"Found {key_hash} which is {self.collection[key_hash]}. Looking for {key} in this collection")
            print(key in self.collection[key_hash])
            if key in self.collection[key_hash]:
...
my_table = HashTable()
my_table.add('fcc', 'flower')
#print(my_table.lookup("fcc"))
print(my_table.lookup("cfc"))
