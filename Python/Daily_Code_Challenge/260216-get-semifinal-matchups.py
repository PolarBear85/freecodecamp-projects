def get_semifinal_matchups(teams):
    ...
    for entry in teams:
        name, record_str = entry.split(": ")
        w, otw, otl, l = map(int, record_str.split("-"))
        
        total_points = (w * 3) + (otw * 2) + (otl * 1) + (l * 0)
        
        team_data.append({"name": name, "points": total_points})

    print(team_data)

    sorted_teams = sorted(team_data, key=lambda x: x['points'], reverse=True)

    t1 = sorted_teams[0]['name']
    t2 = sorted_teams[1]['name']
    t3 = sorted_teams[2]['name']
    t4 = sorted_teams[3]['name']

    return f"The semi-final games will be {t1} vs {t4} and {t2} vs {t3}."
