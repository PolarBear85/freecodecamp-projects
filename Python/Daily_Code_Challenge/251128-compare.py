def compare(word, guess):
    ...
    word_work = list(word)
    guess_work = list(guess)

    for index,letter in enumerate(guess_work):

        if letter == word_work[index]:
            guess_work[index] = 2
            word_work[index] = ""

    for index,letter in enumerate(guess_work):

        if letter in word_work:
            this_index = word_work.index(letter)
            word_work[this_index] = ""
            guess_work[index] = 1
    ...
    return "".join(map(str,guess_work))
