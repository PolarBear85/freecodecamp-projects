function numberOfVideos(videoSize, videoUnit, driveSize, driveUnit) {
  ...
  if (!fileSizes.hasOwnProperty(videoUnit)) return "Invalid video unit"
  if (!driveSizes.hasOwnProperty(driveUnit)) return "Invalid drive unit"


  const videoTotal = videoSize * fileSizes[videoUnit]
  const driveTotal = driveSize * driveSizes[driveUnit]

  console.log(videoTotal,driveTotal)

  const totalVideos = Math.floor(driveTotal/videoTotal)
