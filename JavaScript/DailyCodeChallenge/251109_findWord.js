function findWord(matrix, word) {
  ...
  for (let r = 0; r< rows; r++) {
    ...
    //forward
    let startCol = rowStr.indexOf(word);
    if (startCol !== -1) return [[r,startCol],[r,startCol+word.length-1]]
    //backward
    startCol = rowStr.indexOf(reversedWord);
    if (startCol !== -1) return [[r,startCol + word.length - 1],[r,startCol]]

  }

  for (let c = 0; c<cols; c++) {
    const colStr = matrix.map(row=> row[c]).join("")
    console.log(colStr)
  

    let startRow = colStr.indexOf(word)
    if (startRow !== -1) return [[startRow,c],[startRow+word.length-1,c]]

    startRow = colStr.indexOf(reversedWord)
    if (startRow !== -1) return [[startRow+word.length-1,c],[startRow,c]]
