function compare(word, guess) {
  ...
  const wordWork = word.split("")
  const guessWork = guess.split("")

  for (let x = 0; x < guessWork.length; x++){
    if (wordWork[x]==guessWork[x]) {
      guessWork[x]= 2
      wordWork[x]=""
    }
  }
  console.log("JAM")
  for (let x = 0; x<guessWork.length;x++) {
    console.log(wordWork.includes(guessWork[x]))
    if (wordWork.includes(guessWork[x])) {
      const thisIndex = wordWork.indexOf(guessWork[x])
      guessWork[x] = 1
      wordWork[thisIndex] = ""
    } else if (guessWork[x] != "2") {
      guessWork[x] = 0
    }
  ...
  console.log(guessWork.join(""))
  console.log(wordWork)

  return guessWork.join("")
}
...
Tests:
    ...
    Passed: 1. compare("APPLE", "POPPA") should return "10201".
    Passed: 2. compare("REACT", "TRACE") should return "11221".
    Passed: 3. compare("DEBUGS", "PYTHON") should return "000000".
    Passed: 4. compare("JAVASCRIPT", "TYPESCRIPT") should return "0000222222".
    Passed: 5. compare("ORANGE", "ROUNDS") should return "110200".
    Passed: 6. compare("WIRELESS", "ETHERNET") should return "10021000".
*/