function findPawnMoves(position) {
  ...
  const [col,rawRow] = position
  const row = parseInt(rawRow)
  console.log(col,row)
  if (row===8) return []

  if (row === 2) {
    return [`${col}${parseInt(row)+1}`,`${col}${row+2}`]
  } else {
    return [`${col}${row+1}`]
  }
...
Tests
Passed:1. findPawnMoves("D4") should return ["D5"].
Passed:2. findPawnMoves("B2") should return ["B3", "B4"].
Passed:3. findPawnMoves("A7") should return ["A8"].
Passed:4. findPawnMoves("G2") should return ["G3", "G4"].
Passed:5. findPawnMoves("E3") should return ["E4"].
